...
export default function MusicLyrics() {
  ...
  const [currentLanguage, setCurrentLanguage] = useState<'zh' | 'en' | 'ja' | 'yue'>('zh');
  ...
  const exportLyrics = useCallback((format: 'txt' | 'md') => {
    ...
    if (format === 'md') {
      ...
      content += `**語言**: ${currentLanguage === 'zh' ? '中文' : currentLanguage === 'en' ? 'English' : currentLanguage === 'ja' ? '日本語' : '粵語'}\n\n`;
      content += `---\n\n${lyrics}`;
    } else {
      content = `${title}\n`;
      content += `專輯: ${selectedSong.album || '未知'}\n`;
  ...
  // 切換語言
  const handleLanguageChange = useCallback((language: 'zh' | 'en' | 'ja' | 'yue') => {
        ...
        <div className="lg:col-span-2">
          {selectedSong ? (
            <Card>
              ...
              <CardContent>
                <Tabs value={currentLanguage} onValueChange={(value) => handleLanguageChange(value as 'zh' | 'en' | 'ja' | 'yue')}>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h4 className="font-medium text-gray-900 dark:text-gray-100">
                          正在播放: {currentLanguage === 'zh' ? '中文版' : currentLanguage === 'en' ? '英文版' : currentLanguage === 'ja' ? '日文版' : '粵語版'}
                        </h4>
